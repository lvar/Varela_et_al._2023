...
fitBM = fitContinuous(tree,data[c(1,3)],model="BM")
fitOU = fitContinuous(tree,data[c(1,3)],model="OU")
fitWN = fitContinuous(tree,data[c(1,3)],model="white")
...
#PGLS using OU model
model1<-gls(LogQi~LogBm,data=data,correlation = corMartins(value=3, phy=tree), method = "ML")
anova(model1)
summary(model1)
model2<-gls(LogQi~LogBm*Group,data=data,correlation = corMartins(value=3, phy=tree), method = "ML")
anova(model2)
summary(model2)
model3<-gls(LogQi~LogBm+Group,data=data,correlation = corMartins(value=3, phy=tree), method = "ML")
anova(model3)
summary(model3)
...
mvBM = mvBM(tree=tree.prun, data=data.MMR[c(1,3,5)], error = NULL, model = "BM1")
mvOU = mvOU(tree=tree.prun, data=data.MMR[c(1,3,5)], error = NULL, model = "OU1", param = list(root=FALSE, sigma = NULL, alpha = NULL, vcv = "fixedRoot", decomp = "spherical", decompSigma = "spherical"), method = "rpf", scale.height = TRUE, optimization = "L-BFGS-B", control = list(maxit = 100000), precalc = NULL,
...
#Estimate MMR values 
data.estim = data.MMR
estim = estim(tree=tree.prun, data=data.estim[c(1,3,5)], object=mvOU, error=NULL, asr=FALSE)
estim$estimates
...
data.cross = data.MMR
data.cross[,5] = NA
estim.cross = estim(tree=tree.prun, data=data.cross[c(1,3,5)], object=mvOU, error=NULL, asr=FALSE)
Lower.cross = estim.cross$estimates - estim.cross$se*1.96
Upper.cross = estim.cross$estimates + estim.cross$se*1.96

cross = cbind.data.frame(data.MMR[,5L],estim.cross$estimates[,3L])
...
anc = estim(tree=tree.prun, data=data.estim[c(1,3,5)], object=mvOU, error=NULL, asr=TRUE)
...
ggplot(estimMMR) +
  ...
  geom_point( aes(x=taxa, y=(10^data.MMR[,5]/(10^data.MMR[,3])^0.67)), color="gray0", alpha=1, size=2 ) +
  coord_flip()+
  theme_ipsum() +
